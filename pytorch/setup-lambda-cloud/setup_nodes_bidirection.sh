...
echo "Add private key to the worker nodes ------------------------------"
for IP in ${WORKER_IP[*]}; do
    ssh -i $LAMBDA_CLOUD_KEY ubuntu@$HEAD_IP "scp ~/.ssh/id_rsa ubuntu@$IP:~/.ssh/id_rsa"
done
...
echo "Let all nodes ssh into the head node at least once so in the future it won't ask about fingerprint ------------------------------"
for IP in ${ALL_IP[*]}; do
    ssh -i $LAMBDA_CLOUD_KEY -t ubuntu@$IP "echo exit | xargs ssh ubuntu@$HEAD_IP"
done
